...
import { PAIN_POINTS } from '../config/siteConfig';

export default function ProblemSection() {
  return (
    <section className="bg-gradient-to-b from-[#F5EFE5] via-[#EFE7DA]/60 to-[#F5EFE5] text-[#3A2A20] py-20 lg:py-28 relative overflow-hidden border-b border-[#C9A77A]/25 grain-overlay vignette corner-accent-tr divider-line-top">
      {/* Background Glow Overlay */}
      <div className="absolute top-1/4 right-0 w-96 h-96 bg-[#C9A77A]/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
        ...
        <div className="max-w-3xl mx-auto text-center space-y-4">
          <div className="font-heading inline-flex items-center space-x-2 bg-[#A65F3F]/10 border border-[#A65F3F]/20 text-[#A65F3F] px-4 py-1.5 rounded-xl text-xs uppercase tracking-widest font-semibold">
            <span>Tantangan Pengadaan Rempah B2B</span>
          </div>

          <h2 className="font-heading text-2xl sm:text-3xl lg:text-4xl font-semibold text-[#3A2A20] leading-tight">
            Kesulitan Mendapatkan Pasokan Rempah yang Sesuai Kebutuhan?
          </h2>

          <p className="font-body text-base sm:text-lg text-[#806653] leading-relaxed max-w-2xl mx-auto font-normal">
            Setiap bisnis memiliki kebutuhan rempah yang berbeda. Jenis produk, jumlah, kualitas, hingga kebutuhan penggunaannya perlu disesuaikan sebelum melakukan transaksi.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
          {PAIN_POINTS.map((item) => (
            <div
              key={item.id}
              className="bg-white/90 backdrop-blur-xs p-8 rounded-2xl border border-[#C9A77A]/30 shadow-card hover:shadow-card-hover transition-all duration-300 flex flex-col justify-between group hover:-translate-y-1"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="font-heading text-xs font-mono text-[#A65F3F] tracking-widest uppercase font-bold bg-[#A65F3F]/10 px-2.5 py-1 rounded-lg">
                    MASALAH {item.id}
                  </span>
                </div>

                <h3 className="font-heading text-lg font-semibold text-[#3A2A20] leading-snug group-hover:text-[#A65F3F] transition-colors">
                  {item.title}
                </h3>
              ...
              <div className="pt-6 mt-6 border-t border-[#C9A77A]/15">
        ...
        <div className="mt-14 max-w-2xl mx-auto bg-[#3A2A20] text-[#F5EFE5] p-6 sm:p-8 rounded-2xl text-center border border-[#C9A77A]/30 shadow-xl relative overflow-hidden">
