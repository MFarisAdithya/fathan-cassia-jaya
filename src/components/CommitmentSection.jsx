...
import { GUARANTEES } from '../config/siteConfig';
...
export default function CommitmentSection() {
  ...
  return (
    <section className="bg-gradient-to-b from-[#F5EFE5] via-[#EFE7DA]/50 to-[#F5EFE5] text-[#3A2A20] py-20 lg:py-28 relative overflow-hidden border-b border-[#C9A77A]/25 grain-overlay vignette corner-accent-tr divider-line-top">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#C9A77A]/12 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
        ...
        <div className="max-w-3xl mx-auto text-center space-y-4">
          <div className="font-heading inline-flex items-center space-x-2 bg-[#A65F3F]/10 border border-[#A65F3F]/20 text-[#A65F3F] px-4 py-1.5 rounded-xl text-xs uppercase tracking-widest font-semibold">
            <ShieldCheck className="w-4 h-4 text-[#A65F3F]" />
            <span>Garansi & Perlindungan B2B</span>
          </div>

          <h2 className="font-heading text-2xl sm:text-3xl lg:text-4xl font-semibold text-[#3A2A20] leading-tight">
            Garansi Resmi Kualitas & Kesesuaian Pasokan
          </h2>

          <p className="font-body text-base sm:text-lg text-[#806653] font-normal leading-relaxed max-w-2xl mx-auto">
            Jaminan kepastian mutu dan keamanan transaksi bagi setiap mitra industri, distributor, dan pedagang rempah.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-16 max-w-5xl mx-auto">
          {GUARANTEES.map((item, idx) => (
            <div
              key={item.id}
              className="bg-white rounded-2xl p-8 border border-[#C9A77A]/35 shadow-card hover:shadow-card-hover transition-all duration-300 space-y-4 group hover:-translate-y-1 relative overflow-hidden"
            >
              <div className="flex items-center justify-between">
                <div className="w-12 h-12 rounded-xl bg-[#F5EFE5] border border-[#C9A77A]/30 flex items-center justify-center group-hover:bg-[#A65F3F] group-hover:text-white transition-colors">
                  <div className="group-hover:text-white transition-colors">
                    {icons[idx]}
                  </div>
                </div>
                <span className="font-heading text-xs font-semibold uppercase tracking-wider text-[#A65F3F] bg-[#A65F3F]/10 px-3 py-1 rounded-full border border-[#A65F3F]/20">
                  {item.badge}
                </span>
              </div>

              <div className="space-y-2">
                <h3 className="font-heading text-lg font-semibold text-[#3A2A20] group-hover:text-[#A65F3F] transition-colors">
                  {item.title}
                </h3>
              ...
              <div className="pt-2 flex items-center space-x-2 text-xs font-heading text-[#A65F3F] font-semibold">
                <CheckCircle2 className="w-4 h-4 text-[#A65F3F]" />
                <span>Terjamin 100% Resmi</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 text-center max-w-2xl mx-auto bg-[#3A2A20] text-[#F5EFE5] p-6 rounded-2xl border border-[#C9A77A]/30 shadow-xl">
